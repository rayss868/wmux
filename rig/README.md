...
| id | 구성 | 핵심 어서션 | 상태 |
|---|---|---|---|
| S1 | flood ×8 (페르소나 간 동시 발사, 내부 순차) | 전 도달·seq 연속·무중복(getMessages 전수 대조) + 페르소나 내부 순서 보존 | 이 PR |
| G6 가드 | 하네스 유닛(데몬 불요) | rpc/channelRpc 신원 위생 throw + 정당한 크로스-ws 참조 비차단 | 이 PR |
| S2~S8 | — | — | 후속 PR (PR-R2) |
