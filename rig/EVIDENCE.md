...
## J2 — diff 재직렬화 no-newline 마커 탈락 (adoption-atomicity 리그, 2026-07-10)

J2 채택 리그(`rig/j2/adoption-atomicity.rig.test.ts`)가 **재직렬화 급소(no-newline
마커 탈락 = 조용한 코드 오염)** 를 독립 git 오라클로 잡는다는 실물 증거(설계 §5 — 출하
블로커).

| 항목 | 값 |
|---|---|
| 급소 | 선택 hunk 재조립 시 `\ No newline at end of file` 마커 탈락 → git이 파일 말미에 개행을 붙여 조용히 코드 오염 |
| 잡는 시나리오 | `rig/j2/adoption-atomicity.rig.test.ts` — `it('no-newline 파일 채택 — 파일 말미 개행 오염 없음(실검출 급소)')` |
| 오라클 | 독립 — 적용 후 타겟 파일을 실제 파일시스템에서 읽어 `endsWith('\n') === false` 검사(파서·핸들러 리포트 미의존) |
| 결함 주입 | `WMUX_RIG_J2_DROP_NONEWLINE=1` — 재조립 결과에서 `\ No newline` 라인을 필터링(파서 자체는 무변경 — 리그 전용 흉내) |
| 실증 일자 | 2026-07-10 |
...
주입 ON — 리그 red:
```
$ WMUX_RIG_J2_DROP_NONEWLINE=1 npx vitest run --config vitest.rig.config.ts rig/j2
 × no-newline 파일 채택 — 파일 말미 개행 오염 없음(실검출 급소)
Error: Command failed: git apply .../p.diff
...
주입 OFF(원복) — green:
```
$ npx vitest run --config vitest.rig.config.ts rig/j2
 Test Files  1 passed (1)
      Tests  3 passed (3)
```
...
재직렬화 = 조용한 코드 오염(설계 §7 최악 리스크). no-newline 마커는 unified diff에서
파일 말미 개행 유무를 표현하는 유일한 신호다. 재조립이 이를 탈락시키면 결과 패치는
`git apply`가 거부하거나(컨텍스트 불일치) 통과하되 말미 개행을 오염시킨다 — 어느 쪽이든
독립 오라클(실 파일 재검사)이 red를 낸다. 원문 보존 파서(§3)가 이 마커를 바디 원문으로
통과시키므로 정상 경로는 green이다.
