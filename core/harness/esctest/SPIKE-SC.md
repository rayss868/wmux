...
- report.json은 xterm.js 산출물이라 커밋 가능 — **리뷰 반영으로 rawLogTail(esctest 로그 원문)은
  커밋 산출물에서 제거**(GPL 유래 텍스트 배제 — 집계·요약 수치만). 라이브 디버깅은
  `WMUX_ESCTEST_DEBUG_LOG=1`.
- vendor 부재 시 T1/T2는 명시 skip(T3·파서 단위는 vendor 무관 항상 실행).

---

## 9. 리뷰 로그 — 2모델 적대 패널 (2026-07-09)

Codex 9건 + Claude 5건(실행 검증: 체크섬 파이썬 독립 재계산 재현·92/92를 vendor `def test_`
카운트+esctest 자체 요약 라인과 대조 확정·적대 로그로 파서 누수 부재 확인·WINOPS가 live 값임을
확인). GLM 529 skip. 전건 반영:

| 발견 | 반영 |
|---|---|
| [Codex C9.5+Claude I6] skip·known-bug를 pass로 합산 — 총계 순도 불투명 | status에 `skipped`/`known-bug` 분리 + totals 분리 집계(재생성 결과: 92는 전부 순수 pass — 실피해 0 확인) |
| [Codex C9.2] gen-report 예외 시 파일이 totals에서 소실 | 예외를 합성 error 결과로 기록 + includes/results 개수 불일치 시 exit 1 |
| [Codex C9.0] timeout·nonzero exit가 총계 밖 | totals에 timedOutRuns/nonzeroExitRuns/unreconciledRuns + gen-report가 비정상 시 exit 3 |
| [Codex C8.8→Claude가 "live 값" 확인으로 부분 기각, 잔여=resize 미반영] | WINOPS 문자 resize(CSI 8;r;c t) 가로채 term.resize+child.resize 반영(tryParseWinopsResize 신설) |
| [Codex C8.6+Claude I8] zero-match·요약 대조 부재 — 분모 축소 은폐 | esctest 요약 라인(`*** N tests passed…`) 파싱 → `reconciled` 필드로 케이스별 교차 대조 + zero-match는 error 기록 |
| [Codex C8.4] fetch 재사용이 dirty worktree·origin 미검증 | clean worktree+origin URL 검증 추가(어긋나면 재클론) |
| [Codex I8.3] T3 오라클 순환성 우려 | Claude 독립 파이썬 재계산으로 전 값 재현(0xFFBF·0xFF7D·0xFE0C·0xFF80) — 순환 아님 확정, 주석 유지 |
| [Codex I7.8] rawLogTail에 GPL 유래 로그 텍스트 | 커밋 산출물에서 제거 — 수치 요약(esctestSummary)만, 디버그는 env로 stderr |
| [Codex I7.6+Claude I8] pending 버퍼 무제한(O(n²) 재파싱) | 4KiB 초과 미완결은 선두 강제 배출 후 재파싱(현실 CSI 상한 근거) |
| [Claude I7] watchdog kill 1회 — 신호 무시 시 무한 대기 | SIGKILL 에스컬레이션(2s 유예) + exited를 hardTimeout+5s와 race(무조건 반환) |

재검증(반영 후): `npm run test:harness` **62 passed** · report.json 재생성 totals
`pass=92 knownBug=0 skipped=0 unreconciledRuns=0 decrqcra=1052 winops=125` — 리뷰 전 수치와
정합(분리 후에도 92는 전부 순수 pass — Claude 대조 검증과 일치).
