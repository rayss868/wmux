...
describe('selectProjectCwdSignature', () => {
  ...
  it('is STABLE when a non-cwd metadata field churns (title/agentStatus)', () => {
    const before = ws('a', leaf('l', [surface('x')]), { metadata: { cwd: 'C:/a', title: 'old' } });
    const after = ws('a', leaf('l', [surface('x')]), { metadata: { cwd: 'C:/a', title: 'NEW', agentStatus: 'running' } });
