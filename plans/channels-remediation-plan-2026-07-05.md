...
- 6g. **(P5 델타 리뷰 이월, 2026-07-05 — 의도된 트레이드오프 2건의 후속 옵션):** ①**Discover 미리보기 정적화** — P5가 activeWs 표시 분기를 제거하면서(프라이버시 누출/팬텀 배지 수정) 미가입 공개 채널 미리보기의 라이브 append가 사라짐(히스토리는 열 때 로드). 수용 결정(라이브 필요 시 Join 1클릭). 원하면 미리보기 중 주기 재-fetch 또는 catalog 신호 기반 히스토리 리프레시로 보완 — 표시 규칙 특례는 금지(누출 재개방). ②**에이전트 생성 private 채널의 사람 접근 경로 부재** — ws-human은 초대 불가(유령 행 벡터) + private 자가가입 불가. 해법은 초대 재허용이 아니라 **GUI 오퍼레이터 join 오버라이드**: 신뢰 경로(channels:mutate-local)만 쓰는 데몬 operator-join 능력으로 사람이 private 채널에 스스로 들어가는 명시적 어포던스. 설계 필요.
