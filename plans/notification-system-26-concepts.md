...
**업계 패턴 분석**:
Claude Code 공식 PostToolUse/Stop hook을 `~/.claude/settings.json`에 자동 등록해서 named pipe로 신호 받는 방식. NotificationBell (titlebar 종 + 배지), NotificationRing (pane 테두리), workspace별 unread + 200개 cap + read-first eviction 등의 디자인 패턴 검토.

**구현 원칙**: 외부 코드 차용 없음. 컨셉/패턴만 참고하여 자체 설계로 구현.
...
1. 26개 컨셉을 **검토**하고 다음을 답해주세요:
   ...
   - 외부 코드 차용 없이 자체 구현 관점에서 **위험한 패턴**이 있는가?
...
**OUTSIDE VOICE:** Skipped (both reviews) — the 26-concept candidate list itself functioned as an industry-pattern review surface; CEO surfaced cross-model tension on hook timing and surface scope, user resolved both. No second codex pass needed.
...
- Implementation approach: no external code reuse; concepts only.
