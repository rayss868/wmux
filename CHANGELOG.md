...
- **검증 리그 하네스 코어 + SIM 스모크 (§6.G, PR-R1) — dev-facing.** 자가 검증 하니스의 첫 착지: 최상위 `rig/`에 런 격리(`isolation.ts` — 런당 fresh 임시 홈 + 4-env + `WMUX_DATA_SUFFIX='-rig-{runId}'`)·헤드리스 데몬 하니스(`daemon.ts` — `dist/daemon-bundle` 스폰·`daemon.ping` ready 폴링·SIGKILL·재스폰·번들 부재 시 명시 에러)·데몬 파이프 클라이언트(`pipe.ts` — 지속 소켓 JSON-RPC + 이중 ok 계층 언랩 + G6 정직-main 규율: 페르소나당 workspaceId 1개 바인딩, 타 ws·예약 신원 자칭은 throw)·상태 어서션(`assert.ts` — seq 무결성·본문 전수·unread, 각 헬퍼에 정본 좌표 주석)·결정적 시드(`seed.ts`)를 도입하고, SIM 시나리오 S1(flood ×8 연사 → getMessages 전수 대조: 전 도달·seq 연속·무중복)을 vitest 세 번째 레인(`vitest.rig.config.ts`, `npm run test:rig:sim`)으로 붙였다. 제품 코드(src/) 변경 0 — 데몬 파이프 실물 표면만 사용. 기존 두 테스트 레인 무영향(리그 글롭은 최상위 `rig/**/*.rig.test.ts`만).
...
- **A2A completion-evidence gate activated (§6.M P1, PR-B).** `completed`/`failed` A2A task transitions now require structured completion evidence: `completed` needs a non-empty summary plus at least one well-formed item (`command`/`inspection`/`artifact`), and `failed` needs a summary (the failure reason). The daemon `A2aTaskService.transition` is the single enforcement point; the renderer fallback writer applies the same gate for the paths the daemon soft-defers to it (pane-pinned tasks driven by a pane-identity caller) or when the daemon is unavailable. Rejections return actionable `completion_evidence_*` reason codes (e.g. `completion_evidence_missing`, `completion_evidence_no_items`, `completion_evidence_empty_summary`, `completion_evidence_invalid_item`, `failure_reason_missing`) and leave the task state unchanged with no log append. `verifiedItemCount` stays an honest grade rather than a gate requirement — an all-unverified completion is still accepted (grade 0). Workspace-teardown force-fail and verbatim application of daemon commits intentionally bypass the gate to avoid split-brain.
...
- **무거운 SPA(Gemini 등)를 백그라운드 브라우저 서피스에서 자동화할 수 있다 — `browser_snapshot`이 ref를 노출하고 `browser_press_key`가 실제 keydown을 발생시킨다 (#353).** 백그라운드 브라우저 패널은 `display:none`으로 렌더되어 게스트에 레이아웃이 없고, 그래서 CDP 접근성 트리가 `RootWebArea`만 남는 root-only로 붕괴했다 — `browser_snapshot`이 Gemini의 입력창·전송 버튼 같은 상호작용 ref를 하나도 내주지 못해 ref 기반 자동화가 막혔다. 이제 `generateSnapshot`은 root-only 트리를 감지하면(포그라운드 heavy SPA가 렌더 도중 잠깐 비는 경우를 위해 250ms 재시도 1회 후) 레이아웃이 필요 없는 DOM 셀렉터 스냅샷으로 폴백해 `data-wmux-ref`로 태깅한 상호작용 목록을 `ai`·`aria` 두 포맷 모두에서 반환한다. `resolveRef`는 a11y refMap으로 못 풀면 `data-wmux-ref` 속성으로 폴백해, 스냅샷 백엔드가 세션 중 a11y⇄DOM으로 뒤바뀌어도 "ref not found"가 나지 않는다(백엔드 플랩 제거). DOM 스냅샷 표현식은 두 호출부(RPC 폴백·root-only 폴스루)가 공유하며, 재태깅 전 오래된 `data-wmux-ref`를 먼저 지워 ref 중복을 막는다.
- **`browser_press_key`가 포커스 독립적으로 신뢰된 키 이벤트를 웹뷰에 전달한다 (#353).** 이전 RPC 경로는 이름 없는 키(일반 문자 `z` 등)를 CDP `type:'char'`로 보냈는데, `char`는 텍스트를 삽입할 뿐 DOM `keydown`을 만들지 않아 페이지의 keydown 리스너가 아무것도 못 받았다. 이제 키를 `[Modifier+]*Key`로 파싱해 인쇄 가능 문자는 `text`를 실은 `keyDown`(keydown 발생 + 삽입) + `keyUp`으로, 이름 있는 키(Enter·Tab·Escape·Arrow*·Home/End/PageUp/PageDown·F1~F12·Space·Delete·Backspace)는 실제 키코드로, `Control+a`·`Meta+c` 같은 조합은 CDP modifier 비트마스크로 합성한다(Ctrl·Meta 조합은 단축키 의미이므로 text 억제). 다중 문자 텍스트는 `browser.type.cdp`로 안내하며 거부한다. 또한 게스트 등록 시 `Emulation.setFocusEmulationEnabled`로 포커스를 에뮬레이트하고 백그라운드 스로틀링을 끄므로, 백그라운드 서피스도 입력·접근성·`document.hasFocus()`에서 포커스된 것처럼 동작하고 백그라운드 스크린샷·evaluate 흐름이 스로틀로 멎지 않는다.
