...
- **Git 탭에서 워크트리를 로컬 base로 안전하게 "머지".** 각 feature 워크트리 행에 **Merge** 버튼이 생겼습니다. 누르면 여러분의 로컬 main(또는 repo 기본 브랜치)을 절대 건드리지 않고, base에서 분기한 격리 워크트리에서 git-native 머지를 수행합니다. 충돌이 없으면 자동으로 검증 게이트(`npm test` + `npm run lint`)를 돌리고, **통과했을 때만** "변경 N개 파일 · 검증 통과"라는 한 줄 요약과 함께 **Land**(base를 결과로 fast-forward) / **Discard**(되돌리기) 버튼을 제시합니다. 충돌이 감지되면 자동으로 해결하지 않고 "Conflict — open with Claude" 버튼으로 그 격리 워크트리를 새 워크스페이스로 열어 직접(Claude로) 해결하도록 안내합니다. 검증 통과 여부는 "다 됐다"는 텍스트가 아니라 실제 프로세스 종료 코드로만 판정하며, 진행 중인 머지 상태는 git 디스크에서 파생되어 앱을 재시작해도 Land/Discard를 이어서 할 수 있습니다.
- **Git이 우측 덱 탭으로 합류, 사이드바에 git 신호등 + Git 버튼.** 우측 덱이 Orchestrator·Git·Channels 3탭이 됐습니다. Git 탭은 위에 현재 워크스페이스의 워크트리·PR, 아래에 diff 집계(구 Review — 별도 탭에서 병합)를 한 스크롤로 보여줍니다. 좌측 사이드바 푸터에는 Agent 아래에 **Git 버튼**이 생겨(덱을 열고 Git 탭으로 바로 이동, 커밋 안 된 워크스페이스 수를 배지로 표시), 각 워크스페이스 이름 아래에는 git 신호등 줄이 생겼습니다: 브랜치명과 함께 초록 ●(clean) / 앰버 ●N(커밋 안 된 변경) / 파랑 ↑N(ahead) / 빨강 ↓N(behind)을 색으로 즉시 읽습니다. 하단 diff 집계(Review)는 **지금 보고 있는 repo와 같은 repo**(그 repo의 워크트리 포함)의 워크스페이스만 보여줘 머지 워크플로우와 스코프를 맞췄습니다. 이전 시안(상단 헤더 탭·중앙 전면 표면)은 원복했습니다.
