...
### Added
- **Fleet View — every agent across every workspace on one screen (S-C1).** A new full-screen cockpit overlay (`Ctrl+Shift+A`, or "Open Fleet View" in the command palette) lists every pane across all workspaces as a card grid, with the agents that need you floated to the top: an `awaiting_input` agent (paused mid-turn on a confirmation prompt) sorts first, is outlined, and shows a "needs your input" affordance, while idle terminals sink and dim. A header chip counts how many agents are waiting on you. Click a card — or arrow to it and press `Enter` — to jump straight to that pane, switching workspace + pane + surface in one step (reusing the hardened notification-jump path, including its zoom coherence). The grid is a pure derivation over existing renderer state — no new daemon round-trip — so it reflects live agent status the moment the daemon detects it, and a multi-tab pane surfaces the most-urgent status across its tabs rather than only its active one. The overlay traps focus and is keyboard- and screen-reader-navigable (`role="dialog"`, roving `role="option"` cards). The "Approvals" tab is a stub for the upcoming unified A2A + MCP approval inbox.
