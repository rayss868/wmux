...
Headline (in progress): **Fleet View** — the cockpit. When you are running several agents at once, the hard part is not starting them, it is knowing which one needs you. Fleet View puts every agent across every workspace on one screen and floats the blocked ones to the top, so an unattended fleet becomes a single glance instead of a tab-by-tab hunt.
