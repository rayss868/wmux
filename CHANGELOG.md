...
- **macOS: the sidebar workspace context line (branch, ports, PR) is restored after an app restart.** Reconnecting to a persisted daemon session dropped the session's working directory — and since the metadata poll skips any pane with no cwd, the whole context line collapsed to just the workspace name. The daemon already returns each session's cwd from `listSessions`; reconnect now seeds it. This surfaced as macOS-only because after a reconnect the cwd otherwise only recovers via prompt-scrape, whose patterns match PowerShell (`PS C:\…>`) and bash (`user@host:…$`) but not macOS's default zsh prompt (`host%`), which also emits no OSC 7.
