...
- **검증 리그 시뮬레이터 완성 + SIM 실검출 실증 (§6.G, PR-R2) — dev-facing.** PR-R1 하네스 위에 합성 멀티에이전트 시뮬레이터를 완성했다: 페르소나 프레임워크(`rig/harness/persona.ts` — 신원 배정·채널 서막·시드 배선·수명 관리, 행동 스크립트는 시나리오 소유)와 시나리오 S2~S8(각 격리 데몬, 전부 고정 루프라 결정적 — rng는 S1 flood 전용)을 붙였다. **S2** 핑퐁 부하 하 채널 무결성만(anti-loop 어서션 없음 — 서버측 계약 부재), **S3** dead 소멸 후 unread·멤버십·메시지 원장 잔재 + 채널 기능 잔존(unread 기대값은 클라이언트-측 커서로만 계산 — 데몬 산출을 기대 입력에 넣지 않아 커서 순환 회피), **S4** hung 무응답 하 무한 홀드 없음(정상 post 즉시 커밋) + unread 정확(넛지 어서션 없음 — live PTY 전제), **S5** 현행 deliveryStatus 영수증 계약 고정(ack로만 pending→delivered, 정본 좌표 주석), **S6** 캡 경계±1 와이어 레벨(본문 8192·멘션 64·완료증거 E12 — item 문자열 4096 초과는 게이트 too_large, item 개수 64 초과는 wire normalize malformed), **S7** flood 중 데몬 SIGKILL→재스폰 후 단방 부분집합({RPC ok 커밋}⊆replay — at-least-once tail 승격 계약상 미커밋 무부활은 미어서트), **S8** A2A 전 수명주기(send→working→completed·게이트 거부→재시도·멱등 재전송) + #354 멱등-authz 순서 결함 검출(비참여자 키 재생 차단) + EPERM 카오스(unix 소켓 chmod 000 → 클라이언트 격리·데몬 생존·복구, root는 DAC 우회라 skip). 실검출 실증 ①단(`rig/EVIDENCE.md`): #354 픽스(`2264c4a`)를 스크래치 브랜치에서 revert → S8 red 재현 → main green 확인(CL7 조기 게이트 개방). 도그푸드 29본 흡수 카탈로그(`rig/CATALOG.md` — absorb 4·keep 24·retire 1, 물리 삭제 0). 제품 코드(src/) 변경 0. 기존 두 테스트 레인 무영향.
