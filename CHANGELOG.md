...
- **Quick-action chips above the Commander composer: the commands you run ten times a day are now one click.** The Commander tab grows a small row of chips right above the composer — **Fleet status** asks the orchestrator to read every agent pane's screen and report, per pane, what it's working on and whether it needs your attention; **PR status** has it check your open pull requests (the commander has no shell of its own, so it delegates — it runs `gh pr status` through one of your panes and reads the result back, keeping the evidence in a terminal you can jump to); and after a reboot a **Recover fleet** chip appears alongside the greeting card, so the one-click recovery stays reachable even if you dismissed the card. Chips disable while a turn is streaming, same as the composer.

- **One click brings your whole fleet back after a reboot.** When wmux comes back up after a reboot (or any shutdown that interrupted running agents), the Commander tab now greets you with a recovery card: "*N* agent panes were running before the last shutdown and can be recovered", listing the panes. One click on **Recover fleet** hands the commander a precise per-pane recovery plan — it types each pane's exact resume command (`claude --resume <session>` when the original conversation is known, the safe fallback otherwise), restores each agent's recorded permission mode (a `--dangerously-skip-permissions` fleet comes back in bypass mode, not stuck on prompts — your click on the button is the explicit consent), confirms each agent came back, and reports what every one of them was working on. Typing "recover the fleet" into the composer works too. The per-pane resume pills are still there if you prefer to bring agents back one at a time.

- **The Commander now remembers your conversation across app restarts.** Closing wmux (or rebooting) no longer wipes the orchestrator's memory: the commander's session is persisted on disk, and the next time you send it a message it resumes the same conversation — everything you told it, what it did with your fleet, and how it named things all carry over. Its session storage is also pinned to a stable location, so updating wmux to a new version doesn't break the thread either. If the saved session can't be resumed (e.g. its transcript was cleaned up on the Claude side), the commander quietly starts a fresh conversation instead of erroring on every message.

- **The Commander tab now has a brain: tell it what you want and it runs your fleet for you.** The Command Deck's Commander tab is no longer only a fan-out composer — write a plain message with *no* `@`-mention and it goes to an orchestrator that can see your whole fleet and act on it: it lists and reads your panes, spawns new ones, sends them instructions, and coordinates them over channels/A2A, then streams a running summary back into the thread. Its prose streams in live, and every tool it uses shows up as a chip (green when it succeeded, red when it failed) — chips that touched a specific pane are clickable, so one click jumps the fleet straight to the evidence. A **Stop** button interrupts a turn mid-flight. The orchestrator runs on your Claude subscription (no API key needed) and drives the fleet through the same wmux tools any agent gets, so wmux itself holds no orchestration policy — the model does. `@`-mentioning panes still does the direct Phase 1 fan-out exactly as before. (This first cut can spawn and drive panes but not close them — cleanup stays a manual step for now; inline approval for destructive actions and cross-restart memory are coming next.)

- **The right dock is now a Command Deck: command your whole fleet from one thread instead of typing pane-to-pane.** The dock opens on a new **Commander** tab (the channel list moved one tab over to **Channels**). In Commander you write one message, `@`-mention as many agent panes as you want — the same autocomplete the channel composer uses, so `@` lists every live agent pane across all your workspaces — and hit send. The message fans out to every mentioned pane at once (delivered by the existing plumbing: a running Claude pane gets it immediately, others on their next turn), and each pane's reply lands back in the *same* Commander thread, grouped under the message you sent — no more clicking into each terminal to type the same thing and hunting for answers. The dispatch shows a chip per targeted pane and each reply's author is clickable, so one click jumps the fleet to that pane. Under the hood it's an ordinary private `#commander` channel (it also appears in the Channels tab), so its history is durable and survives restarts like any other channel.
