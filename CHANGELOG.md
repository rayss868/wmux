...
- **Child shells never inherit a stale wmux identity.** A wmux launched from inside a wmux pane (e.g. `npm start` while dogfooding) inherited the parent pane's `WMUX_WORKSPACE_ID` / `WMUX_SURFACE_ID` / `WMUX_SOCKET_PATH` in its own environment, which could survive into freshly created child shells (daemon mode never forces a socket path; some create paths run before a surfaceId exists). The whole reserved `WMUX_*` namespace is now cleared from the spawn baseline before identity is forced, so a child's identity is only ever what wmux explicitly sets — the spoofing guarantee is now unconditional, not profile-only.

### Fixed
- **`browser_extract_data` no longer copies the same value into every field.** On a packaged-build dogfood the tool read the page but filled `title` and `url` with the same row subtext line (e.g. `"117 points by transistor-man 4 hours ago | hide | 39 comments"`) instead of the story title and href. Three gaps are closed: a blank or one-char table header matched every field name (`''.includes` is always true) and collapsed the columns; `tr`-based list layouts (e.g. Hacker News rows) were not recognized as repeated items; and the repeated-element strategy grabbed the first href-less vote anchor instead of the row's primary link. Field resolution now prefers a row's primary link for `title`/`url`, reads `href` for url-like fields, and skips blank headers. Regression coverage runs the in-page callbacks against a jsdom document. ([#110](https://github.com/openwong2kim/wmux/issues/110))
- **`browser_cookies`, `browser_storage`, `browser_emulate`, and `browser_resize` work on packaged builds.** These four state tools depended on a Playwright `Page`, which `getPage()` cannot produce for the embedded `<webview>` on packaged builds, so each failed with `No browser page available` even with a browser panel open and active. They now fall back to the same main-process CDP/RPC channel the extraction and capture tools already use (#105/#106): cookies via the `Network.*` cookie commands, storage via `browser.evaluate`, and viewport/emulation via `Emulation.*`. The Playwright path is unchanged in dev; the fallback engages only when no Page is available. ([#111](https://github.com/openwong2kim/wmux/issues/111))
- **Per-terminal working directory is reported correctly (local and daemon mode).** The tab tooltip and the workspace "Working directories" menu showed each shell's startup home directory (e.g. `C:\Users\me`) for every PowerShell regardless of where it had `cd`'d. Two compounding parser bugs are fixed: OSC 7 left Windows paths as `/C:/Users/me` (leading slash, forward slashes), and prompt detection matched the stale echoed prompt and froze the reported cwd at startup. Parsing is extracted into a unit-tested `cwdDetect` module (shared by both spawn paths) that normalizes the OSC 7 URI to a native path — including UNC shares — and reads the live (last) prompt. Daemon mode additionally never forwarded its detected cwd to the renderer; a new `session:cwd` event now closes that gap so daemon-backed panes live-update like local ones.
- **Tighter workspace right-click menu.** The context menu was pinned to a fixed minimum width, leaving a wide blank gutter beside short items (and an oversized gap before the "Working directories" submenu arrow); it now sizes to its content.
